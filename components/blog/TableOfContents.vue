        ...
        <div class="flex flex-col space-y-4 items-start">
            ...
            <ul class="px-2 space-y-4 text-sm">
                <li v-for="item in content" :key="item.id" class="space-y-4">
                    <a
                        :href="`#${item.id}`"
                        class="text-zinc-800 dark:text-chaos-foreground text-sm lg:text-base"
                        >{{ item.text }}</a
                    >
                    <ul v-if="item.children" class="space-y-4 ml-4">
                        <li
                        ...
                        >
                            <a
                                :href="`#${nestedItem.id}`"
                                class="text-zinc-800 dark:text-chaos-foreground text-xs lg:text-sm"
                                >{{ nestedItem.text }}</a
                            >
                        </li>
                    </ul>
